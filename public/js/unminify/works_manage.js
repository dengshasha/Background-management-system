    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $(NewData).each(function(i,v){
                var newIshot = v.off_line == true ? 'active':'';
                var newNothot = v.off_line == true ? '':'active';
                $('table tbody').append('<tr data-value="'+ v.works_id +'"><td>'+ v.works_id+'</td><td>'+ v.works_name+'</td><td>'+ v.owner+'</td><td>'+ NewDate(v.created_at)+'</td><td><span data-value="true" class="setHot'+' '+newIshot+'">下线</span><span data-value="false" class="setHot'+' '+newNothot+'">取消下线</span></td></tr>');
            })
        },
...
$('#functionBtn').on('click','span',function(){
    searchApiOptions.off_line=$(this).attr('data-value');
...
$('tbody').on('click','span',function(){
    ...
    if(!$(this).hasClass('active')){
        $.ajax({
            type:'PATCH',
            url:'http://cmadmin.api.v.vidahouse.com/v1/works/offline',
            async:false,
            data:{
                off_line:data_value,
                works_id:works_id
...
$("#pager").pager({ pagenumber: 1, pagecount: pageCount, buttonClickCallback: PageClick });
//搜索按回车键
$('#searchCondition').on('keypress',function(e){
    if(e.keyCode==13){
        e.preventDefault();
        searchApiOptions.q= $('#searchCondition').val();
        searchApiOptions.start=0;
